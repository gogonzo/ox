...
ox(\(x) is.character(x) && length(x) == 1, x, .else = y)
```
...
# if (checkmate::test_character(x, len = 2)) x else y
ox(checkmate::test_character, x, len = 2, .else = y)
```
...
library(dplyr)
iris %>%
  dplyr::mutate(x = ifelse(Sepal.Length > Petal.Length, Sepal.Length, Petal.Length))

iris %>% 
  dplyr::mutate(x = OX(`>`, Sepal.Length, Petal.Length))
```
